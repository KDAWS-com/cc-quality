...
> **Note:** `gitleaks` is a pre-commit tool installed by `wflow:setup`, not a runtime dependency of this command.
...
3. **Post results to PR** — New collapsible comment each scan cycle (preserves audit trail):
   ...
   **Dependencies:** {N} CVEs triaged ({N} exploitable, {N} theoretical)
   ...
   ---
   *Generated by `quality:security-review` • Cycle {N}/3*
   </details>
   ```

4. **Block or proceed:**
   - Blocking findings exist → fix directly on the current branch, then re-run `quality:security-review`.
   - All clear → signal ready to proceed.

## Output Contract

When invoked from another plugin (e.g., `wflow`), this command:
